...
class FCUCB_D():
    def __init__(self, L_, K_, mean_reward_, delay_type, reward_type, p_=0,c=4):
        ...
        if isinstance(self.p, int):
            self.arms=[Arm(self.reward_type, self.delay_type,self.mean_reward[a],self.p) for a in range(self.K)]
        else:
    ...
    def select_arms(self, round):
        ...
        ucb = np.minimum(ucb, ones)
        lcb = np.maximum(lcb, zeros)
        mean_reward_est = solve_optimized_problem(lcb,ucb,self.K,self.L,self.c)
        merit_sum_est = sum([Merit_Fun(mean_reward_est[i],self.K,self.L,self.c) for i in range(self.K)])
        pro = [self.L * Merit_Fun(mean_reward_est[i],self.K,self.L,self.c) / merit_sum_est for i in range(self.K)]
    ...
    def get_reward_regret(self, pro):
        reward_regret = reward_regret_each_slot(pro,self.mean_reward,self.K,self.L,self.c)
        return reward_regret

    def get_fairness_regret(self, pro):
        fairness_regret = fairness_regret_each_slot(pro,self.mean_reward,self.K,self.L,self.c)
...
class CUCB_D():
    def __init__(self, L_, K_, mean_reward_, delay_type, reward_type, p_=0,c=4):
        ...
        if self.reward_type == 'Gaussian_reward':
            self.outstanding_reward = [[np.random.normal(self.mean_reward[i], 1)] for i in range(self.K)]
        elif self.reward_type == 'Bernoulli_reward':
            self.outstanding_reward = [[np.random.binomial(n=1, p=self.mean_reward[i], size=1)[0]] for i in range(self.K)]
        else:
            print("wrong reward_type")

        if self.delay_type == 'Packetloss_delay':
            self.arriving_round = [[0] if np.random.uniform(0,1) < self.p else [math.inf] for i in range(self.K)]
        elif self.delay_type == 'Geometric_delay':
            self.arriving_round = [[np.random.geometric(self.p[i], 1)[0]] for i in range(self.K)]
        elif self.delay_type == 'Fixed_delay':
            self.arriving_round = [[self.p] for i in range(self.K)]
        elif self.delay_type == 'Pareto_delay':
            self.arriving_round = [[(np.random.pareto(self.p[i], None) + 1) * 1] for i in range(self.K)]
        elif self.delay_type == 'Undelayed':
            self.arriving_round = [[0] for i in range(self.K)]
        else:
            print("wrong delay_type")
...
class OP_FCUCB_D():
    def __init__(self, L_, K_, mean_reward_, delay_type, reward_type, p_=6000,c=4):
        self.L = L_
        self.K = K_
        self.p = p_
        self.c = c
    ...
    def select_arms(self, round):
        ...
        mean_reward_est = solve_optimized_problem(pessimistic_lcb,optimistic_ucb,self.K,self.L,self.c)
        merit_sum_est = sum([Merit_Fun(mean_reward_est[i],self.K,self.L,self.c) for i in range(self.K)])
        pro = [self.L * Merit_Fun(mean_reward_est[i],self.K,self.L,self.c) / merit_sum_est for i in range(self.K)]
        arm_selected = DepRound(self.L, pro)
...
class OP_FCTS_D():
    ...
    def select_arms(self, round):
        ...
        merit_sum_est = sum([Merit_Fun((mean_reward_est_optimistic[i]+mean_reward_est_pessimistic[i])/2,self.K,self.L,self.c) for i in range(self.K)])
        pro = [self.L * Merit_Fun((mean_reward_est_optimistic[i]+mean_reward_est_pessimistic[i])/2,self.K,self.L,self.c) / merit_sum_est for i in range(self.K)]
        arm_selected = DepRound(self.L, pro)
